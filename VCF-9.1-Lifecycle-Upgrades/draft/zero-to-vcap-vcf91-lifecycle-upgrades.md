...
date: 2026-07-19
tags: [VCF, VMware Cloud Foundation, 9.1, Lifecycle Management, Upgrade, Fleet Lifecycle, SDDC Lifecycle, VCF Operations, SDDC Manager, Aria Suite Lifecycle, Zero to VCAP, HumbledGeeks]
draft: true
---

<!-- ============================================================
GROUND-TRUTH NOTES (read before editing):
- This is a CONCEPT + WALKTHROUGH post about how lifecycle works in VCF 9.1,
  framed against the VCF 8.0 U3 / 5.x two-tool model. It is NOT a troubleshooting
  post. Source of truth is ../SOURCE-NOTES.md.
- Core thesis: old model = two lifecycle systems (SDDC Manager + Aria Suite
  Lifecycle). VCF 9.1 = one control plane, mental model is fleet vs instance.
- Nuance that MUST survive edits: steady state is simpler, but getting TO 9.1 is a
  real project with prerequisites. Do not let this read as "click upgrade and go."
- Companion piece = the VVF vCenter upgrade post (ID 2567). CONFIRMED LIVE
  2026-07-19; both links now use the final slug
  https://humbledgeeks.com/how-easy-is-it-to-patch-vcenter-in-vvf-91-i-applied-a-critical-security-patch-to-find-out/
- IMAGES are placed and captioned (16 shots, paths ../images/NN-folder/). Full
  disposition and sensitive-data findings are in ../IMAGE-MANIFEST.md. See the
  DRAFT NOTES block at the bottom for the security must-fix.
- BEFORE PUBLISH: scan every screenshot for cleartext passwords, license keys, and
...
![The SDDC Manager dashboard, the single-instance console I ran core lifecycle from, with workload domains and upgrades tracked separately from the Aria stack](../images/01-old-way/upgrade_51.jpg)
...
![The same SDDC Manager UI in 9.1, now carrying a deprecation banner that points lifecycle work to VCF Operations. It still runs, but it is on its way out](../images/01-old-way/upgrade_50.jpg)
...
![A component detail view that separates the fleet components FQDN from the instance components FQDN. That split is what decides which services are shared across instances and which are per instance](../images/02-architecture/upgrade_26.jpg)

![Fleet Management in VCF Operations, where the console sees VCF Management, the VCF instances, and any standalone vCenters as one fleet](../images/02-architecture/upgrade_30.jpg)
...
![Change target version opens the Set Target Version dialog, where I pick the fleet target build from the dropdown and set it](../images/03-management-upgrade/upgrade_42.jpg)

![With the target set, every management component resolves to a current-to-target path and reads Ready for upgrade](../images/03-management-upgrade/upgrade_19.jpg)

![Selecting all nine components turns the actions into Run Prechecks and Upgrade for the whole set](../images/03-management-upgrade/upgrade_13.jpg)

![The confirmation dialog is the commit point. It reminds you to pass prechecks first, which the tool means literally](../images/03-management-upgrade/upgrade_14.jpg)

![The Tasks tab breaks the run into named subtasks: set context, stage plugin, run prechecks, stage package, prepare, upgrade, then inventory sync](../images/03-management-upgrade/upgrade_21.jpg)

![Back on the Components tab afterward, every management service reads its new target build. This is where I confirm what actually landed](../images/03-management-upgrade/upgrade_6.jpg)
...
![On the dc3-wld01 workload domain, the Upgrades tab runs its own prechecks before any core component moves. This is the instance-level side, and it only starts after the fleet components are current](../images/04-core-upgrade/upgrade_32.jpg)

<!-- IMAGE GAP: this section only has the precheck shot (upgrade_32). No captures of the
Plan Component Upgrade wizard, Select Components and Target Version, Submit Plan, the
Upgrade Sequence card, or Schedule / Start Now. Grab those on the next core upgrade to
round out this walkthrough. -->
...
![The Software Depot connection mode. Connected pulls from Broadcom online, while Offline Depot and Disconnected cover dark sites](../images/05-depot-binaries/upgrade_48.jpg)

![The Register step ties the depot to Broadcom with an activation code. This is the redacted copy of that screen](../images/05-depot-binaries/upgrade_47.jpg)

![The Software Depot overview shows the connected depot and its storage, which is where downloaded bundles live for the patch planner](../images/05-depot-binaries/upgrade_49.jpg)

![The VCF Installer side of the depot, with the vSphere Foundation and Cloud Foundation bundles already downloaded before any deploy or upgrade](../images/05-depot-binaries/upgrade_56.jpg)
...
- TITLE: "VCF 9.1 Lifecycle: One Control Plane, and the Real Work to Get There"
  (chosen from the option set; folds in the honest-project nuance so it does not
  read as click-upgrade-and-go).
- COMPANION LINK: RESOLVED 2026-07-19. Post 2567 is published; both links use the
  final slug (…/how-easy-is-it-to-patch-vcenter-in-vvf-91-…/). No swap pending.
- IMAGES: placed and captioned. 16 shots selected from the 56 in images/_inbox and
  copied into images/01-old-way through images/05-depot-binaries. Image paths use
  ../images/NN-folder/ because this .md lives in draft/. Confirm the WP build script
  resolves that relative path (the VVF post had md and images in the same dir).
  Full per-image disposition + sensitive-data findings are in
  ../IMAGE-MANIFEST.md.
- CORE-UPGRADE SECTION IS THIN: only upgrade_32 (a precheck). No Plan Component
  Upgrade wizard / Submit Plan / Start Now shots exist. Capture those next core run.
- SCREENSHOT SAFETY, MUST-FIX BEFORE PUBLISH:
  * upgrade_24.jpg (in _inbox, NOT used) exposes a CLEARTEXT VCF Operations admin
    password. Do not publish it, remove it from the repo, and rotate that password.
  * upgrade_29.jpg (installer Review Passwords, masked) is out of scope, not used.
  * Every placed shot carries dc3-*.humbledgeeks.com FQDNs and 10.103.x IPs; several
    carry the admin.allen@humbledgeeks.com UPN and administrator@vsphere.local.
    upgrade_46 (not used; upgrade_47 redacted copy used instead) shows the software
    depot ID. Decide redact-vs-keep to match your prior posts before pushing.
- FOLLOW-UP POST: the "fleet lifecycle sub-tasks stuck In Progress" known issue is
  teased here for a dedicated write-up. Link it back once it exists.
- PUBLISH: image-heavy pattern (build script -> publish.py --no-images ->
  postprocess_wp.py). No post ID yet; draft on disk only.
-->
