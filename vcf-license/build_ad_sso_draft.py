#!/usr/bin/env python3
"""Prepare the VCF Active Directory SSO post for WordPress:
  1. Upload each local screenshot in ad-sso-images/ to the WP media library
     (cached so re-runs reuse).
  2. Emit a publish-ready markdown copy: YAML front matter + HTML comments stripped,
     an H1 injected from the front-matter title (publish.py uses the H1 as the WP
     title), local image paths rewritten to their uploaded media URLs.
Then run:  python scripts/publish.py vcf-license/_build-ad-sso-draft.md --no-images --tags "..."

NOTE: publish.py drops raw HTML comments at the Gutenberg-block step, so the
`<!-- DRAFT NOTES -->` block is re-injected into the draft as a Custom HTML block
by a separate REST patch after publishing. Modeled on vcf-license/build_wp_draft.py.
"""
...
SRC = Path("vcf-license/vcf-active-directory-sso.md")
BUILD = Path("vcf-license/_build-ad-sso-draft.md")
CACHE = Path("vcf-license/.wp-media-ad-sso-map.json")
IMG_DIR = Path("vcf-license")  # refs look like ad-sso-images/<name>.jpg, relative to vcf-license/
...
# strip HTML comments (the DRAFT NOTES block) — re-injected post-publish as wp:html
...
imgs = list(dict.fromkeys(re.findall(r"ad-sso-images/[A-Za-z0-9._-]+\.jpg", body)))
