---
title: "Connecting My FlexPod VCF 9.1 Deployment to Active Directory (VCF Single Sign-On)"
date: 2026-06-28
tags: [VCF, VMware Cloud Foundation, 9.1, SSO, Active Directory, LDAPS, Identity Broker, FlexPod, Cisco UCS, NetApp]
draft: true
---
...
![VCF Operations login page using a local account](ad-sso-images/01-ops-login.jpg)
...
![VCF Operations Launchpad welcome tiles](ad-sso-images/02-launchpad.jpg)

*The Launchpad — Identity and Access lives under the Manage area.*

From the top navigation, head to **Manage**.

![VCF Operations Home with the Manage menu](ad-sso-images/03-home-manage-menu.jpg)
...
![Identity and Access Get Started page with the SSO architecture diagram](ad-sso-images/04-identity-access-getstarted.jpg)
...
![Get Started page with the CONFIGURE SSO button](ad-sso-images/05-getstarted-configure-sso.jpg)
...
![Prerequisites tab with all acknowledgments unchecked](ad-sso-images/06-prerequisites-unchecked.jpg)
...
![Prerequisites tab with all five acknowledgments checked and SUBMIT active](ad-sso-images/07-prerequisites-checked.jpg)
...
![Configure VCF SSO wizard at 0 percent](ad-sso-images/08-configure-sso-0pct.jpg)
...
![Choose Deployment Mode with the Instance card selected](ad-sso-images/09-deployment-mode-instance.jpg)
...
![Choose Identity Broker showing the existing dc3-vidb detected](ad-sso-images/10-deployment-broker-selected.jpg)
...
![Configure VCF SSO at 25 percent, step 1 completed](ad-sso-images/11-configure-sso-25pct.jpg)
...
![Choose identity provider list with the AD/LDAP tile highlighted](ad-sso-images/12-choose-idp-adldap-tile.jpg)

*Pick AD/LDAP under Directory-Based Identity Provider — not Open LDAP.*

With AD/LDAP selected, click **NEXT**.

![AD/LDAP selected with the Next button active](ad-sso-images/13-choose-idp-next.jpg)

Then click **CONFIGURE** on task 2 to open the connection form.

![Configure the identity provider task with the Configure button](ad-sso-images/14-configure-idp-button.jpg)
...
![Blank Directory information form, top](ad-sso-images/15-directory-info-blank-top.jpg)

![Blank Directory information form, lower fields](ad-sso-images/16-directory-info-blank-mid.jpg)
...
![Directory information with display name and primary/secondary DCs filled](ad-sso-images/17-directory-info-top-filled.jpg)
...
![Directory information showing certificates, base DN and the bind DN](ad-sso-images/19-directory-info-certs-binddn.jpg)

![Directory information with the bind password entered](ad-sso-images/18-directory-info-password.jpg)
...
![Directory information review summary](ad-sso-images/20-directory-info-review.jpg)
...
![Configure Identity Provider showing 2 of 4 tasks complete](ad-sso-images/21-configure-idp-2of4.jpg)
...
![Setup Provisioning step 1, Review Directory Information](ad-sso-images/22-provisioning-review-dir.jpg)
...
![Attribute Mappings with standard AD defaults](ad-sso-images/23-provisioning-attr-mappings.jpg)
...
![Group Provisioning, blank](ad-sso-images/24-group-prov-blank.jpg)
...
![Group Provisioning with the base group DN entered](ad-sso-images/25-group-prov-basedn.jpg)

The groups under that OU load into a selectable grid. This is also live proof your
bind account can read the directory.

![Group Provisioning with the group list loaded](ad-sso-images/26-group-prov-list.jpg)
...
![User Provisioning on the first pass](ad-sso-images/27-user-prov-firstpass.jpg)
...
![Validation error: no groups or users selected](ad-sso-images/28-provisioning-error.jpg)
...
![Group Provisioning with Select All enabled](ad-sso-images/29-group-prov-selectall.jpg)
...
![User Provisioning with the base user DN left blank](ad-sso-images/30-user-prov-blank.jpg)
...
![Setup Provisioning review and Finish](ad-sso-images/31-provisioning-review.jpg)

Provisioning shows complete; click **NEXT** to the optional test login.

![Provisioning complete with Next](ad-sso-images/32-provisioning-complete-next.jpg)
...
![Test login step ready](ad-sso-images/33-test-login-ready.jpg)
...
![Browser certificate warning for the broker](ad-sso-images/34-cert-warning.jpg)
...
![VMware Directory Login page with an AD user](ad-sso-images/35-directory-login.jpg)
...
![Test login success page](ad-sso-images/36-test-login-success.jpg)

Back in VCF Operations, the inline banner confirms **"Test login is successful"** —
click **DONE**.

![Configure Identity Provider showing the successful test login and Done](ad-sso-images/37-test-login-done.jpg)
...
![Configure VCF SSO at 50 percent, step 3 ready](ad-sso-images/38-configure-sso-50pct.jpg)
...
![Enable SSO component list](ad-sso-images/39-enable-sso-list.jpg)

*My three: dc3-vc01 (Management Domain), dc3-vc02 (workload), dc3-nsx01 (NSX).*

Select **all three** and click **CONFIGURE**.

![All three components selected with Configure](ad-sso-images/40-enable-sso-selected.jpg)
...
![Confirmation dialog, unchecked](ad-sso-images/41-enable-sso-confirm-unchecked.jpg)

Check the box and click **FINISH**.

![Confirmation dialog checked, Finish active](ad-sso-images/42-enable-sso-confirm-checked.jpg)

The components move to **In Progress** and configure one by one.

![Enable SSO in progress](ad-sso-images/43-enable-sso-inprogress.jpg)

When they finish, the flow reaches 75% and **Assign VCF Roles** unlocks.

![Configure VCF SSO at 75 percent](ad-sso-images/44-configure-sso-75pct.jpg)
...
![Assign VCF Roles, groups list](ad-sso-images/45-assign-roles-groups.jpg)

Select **`VCF_Admins`** and click **ASSIGN**.

![VCF_Admins selected with Assign](ad-sso-images/46-assign-vcf-admins-checked.jpg)

> **Scope it deliberately.** I initially had three "admin" groups selected here.

![Assign page with three groups selected](ad-sso-images/47-assign-three-groups.jpg)
...
![Assign page with only VCF_Admins](ad-sso-images/48-assign-vcf-admins-only.jpg)

Set the **Scope** first — choosing **"Components with dc3-vidb.humbledgeeks.com"**
(fleet-wide) is what unlocks the VCF-level roles.

![Scope dropdown options](ad-sso-images/49-scope-dropdown.jpg)
...
![Role dropdown options](ad-sso-images/50-role-dropdown.jpg)
...
![Scope and Role selected, ready to assign](ad-sso-images/51-scope-role-selected.jpg)

Green banner — **"Changes successful."** `VCF_Admins` now holds VCF Administrator
(effective at next login).

![Roles assigned successfully](ad-sso-images/52-roles-assigned-success.jpg)
...
![Configure VCF SSO at 100 percent](ad-sso-images/53-configure-sso-100pct.jpg)
...
![Finish Setup dialog with Export Configuration](ad-sso-images/54-finish-setup-modal.jpg)
...
![Overview showing VCF Instances 3 of 3 configured](ad-sso-images/55-overview-instances-3of3.jpg)
...
![Overview with VCF Management red, 2 not configured](ad-sso-images/56-overview-management-red.jpg)

On the **VCF Management** tab, tick **both** appliances and click **JOIN VCF SSO**.

![VCF Management tab with both appliances selected and Join VCF SSO](ad-sso-images/57-management-join-selected.jpg)

On **Choose SSO**, the JOIN button is greyed until you pick a broker...

![Join VCF SSO with broker not yet selected](ad-sso-images/58-join-broker-unselected.jpg)

...select **`dc3-vidb.humbledgeeks.com`** and click **JOIN**.

![Join VCF SSO with the dc3-vidb broker selected](ad-sso-images/59-join-broker-selected.jpg)

Acknowledge the role-assignment confirmation and click **FINISH**.

![Join VCF SSO confirmation dialog](ad-sso-images/60-join-confirm.jpg)

Both appliances flip to **Configured**, and **VCF Management Status** turns
**blue — 2 Configured**. No more red.

![VCF SSO Overview fully configured, no red](ad-sso-images/61-overview-all-configured.jpg)
...
*Next post in the series: [Running DOOM on Kubernetes: vSphere Kubernetes Service (VKS) on My FlexPod VCF 9.1](https://humbledgeeks.com/running-doom-on-kubernetes-vsphere-kubernetes-service-vks-on-my-flexpod-vcf-91/).*

<!-- DRAFT NOTES — remove before publishing
Verified end-to-end in lab on 2026-06-28. All 61 screenshots embedded in workflow order.
Redact before publishing: humbledgeeks.com + host FQDNs, svc_vcf_sso DN, allen@humbledgeeks.com,
admin local account, masked passwords, broker self-signed cert warning is expected.
Remaining real-world to-dos: component-level vCenter/NSX role assignments; CA-sign broker cert; export SSO config backup.
-->
