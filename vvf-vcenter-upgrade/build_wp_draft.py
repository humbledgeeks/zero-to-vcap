#!/usr/bin/env python3
"""Prepare the VVF 9.1 vCenter security-patch post for WordPress:
...
Then run:  python scripts/publish.py vvf-vcenter-upgrade/_build-wp-draft.md --no-images --tags "..."

Modeled on vcf-license/build_wp_draft.py (this post has no SVG diagrams, so that step is dropped).
"""
...
SRC = Path("vvf-vcenter-upgrade/zero-to-vcap-vvf91-vcenter-upgrade.md")
BUILD = Path("vvf-vcenter-upgrade/_build-wp-draft.md")
CACHE = Path("vvf-vcenter-upgrade/.wp-media-vvf-map.json")
IMG_DIR = Path("vvf-vcenter-upgrade")  # refs look like images/<name>.jpg, relative to vvf-vcenter-upgrade/
...
# strip HTML comments (ground-truth notes + DRAFT NOTES block)
